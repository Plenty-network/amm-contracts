...
Swap = sp.io.import_script_from_url("file:./volatileSwapDeployer/volatileSwap.py")

class FactoryContract(sp.Contract):

    def __init__(self,_adminAddress): 
        
        self.SwapContract = Swap.AMM()
    ...
    def routerCall(self,exchangeAddress, token1Address, token2Address, token1Id, token2Id, token1Type, token2Type, token1Amount, token2Amount, userAddress):
        ...
        contractData = sp.record(
            ...
            token1Amount = token1Amount, token2Amount = token2Amount, userAddress = userAddress, stablePair = False
        )
...
if "templates" not in __name__:
    ...
    def test():
        ...
        factory = FactoryContract(adminAddress) 
        scenario += factory

        sp.add_compilation_target(
            "volatileDeployer",
            FactoryContract(adminAddress)
        )